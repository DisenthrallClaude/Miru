...
abstract final class Frost {
  ...
  /// 底色不透明度。
  ///
  /// 玻璃必须保住自己的主题身份：亮色下明显发亮、暗色下明显发暗，
  /// 而不是被背后内容「冲刷」。之前取 0.30/0.26 时，选源弹层叠在
  /// 深色封面上整体发灰发暗（亮色主题下观感是「暗」的），时间表
  /// 顶栏被滚过的亮色封面冲成灰白（暗色主题下白字被冲淡）。
  /// 提到 0.72/0.78 后：玻璃仍保留 20%~30% 的通透感（模糊采样的
  /// 色彩呼吸依然可见），但亮度观感与对话框、不透明面板一致，
  /// 全应用弹层/面板/顶栏不再有明暗割裂。
  /// 可读性由「高不透明底 + 模糊 + 边缘高光」共同保证。
  static double tintOpacity(Brightness brightness) =>
      brightness == Brightness.light ? 0.72 : 0.78;
