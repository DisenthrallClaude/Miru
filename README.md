...
- **开屏主题三重判定（Sky 昼 / Astro 夜）**：北京时间 23:00–次日 06:00 的深夜窗口进入开屏，一律切到夜版 Astro 星空——即使应用与系统都是浅色模式；应用内设置「深色模式」或系统深色模式同样切夜版（此前只看系统深色，应用内深色设置被开屏无视）。启动加载底色与开屏同源判定，昼夜颜色全程无缝。
...
- 其余：开屏亮度判定的存储读取带防御（存储损坏时按「跟随系统」处理，不再可能首帧崩溃）；新增 17 项开屏主题判定测试，全量 303 项测试通过。
